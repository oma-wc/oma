...
const phaseSymbol$1 = Symbol('haunted.phase');
const hookSymbol$1 = Symbol('haunted.hook');
const updateSymbol$1 = Symbol('haunted.update');
const commitSymbol$1 = Symbol('haunted.commit');
...
class State$1 {
    constructor(update, host) {
        this.update = update;
        this.host = host;
        this[hookSymbol$1] = new Map();
        this[effectsSymbol$1] = [];
        this[layoutEffectsSymbol$1] = [];
    }
    run(cb) {
        setCurrent$1(this);
        let res = cb();
    ...
    runEffects() {
        this._runEffects(effectsSymbol$1);
    }
    runLayoutEffects() {
        this._runEffects(layoutEffectsSymbol$1);
    }
    teardown() {
        let hooks = this[hookSymbol$1];
        hooks.forEach(hook => {
            if (typeof hook.teardown === 'function') {
                hook.teardown();
            }
...
const defer$1 = Promise.resolve().then.bind(Promise.resolve());
function runner$1() {
    let tasks = [];
    let id;
    function runTasks() {
...
const read$1 = runner$1();
const write$1 = runner$1();
class BaseScheduler$1 {
    constructor(renderer, host) {
        this.renderer = renderer;
        this.host = host;
        this.state = new State$1(this.update.bind(this), host);
        this[phaseSymbol$1] = null;
        this._updateQueued = false;
    }
    update() {
        if (this._updateQueued)
            return;
        read$1(() => {
            let result = this.handlePhase(updateSymbol$1);
            write$1(() => {
                this.handlePhase(commitSymbol$1, result);
                write$1(() => {
                    this.handlePhase(effectsSymbol$1);
                });
            });
            this._updateQueued = false;
        });
    ...
    handlePhase(phase, arg) {
        this[phaseSymbol$1] = phase;
        switch (phase) {
            case commitSymbol$1:
                this.commit(arg);
                this.runEffects(layoutEffectsSymbol$1);
                return;
            case updateSymbol$1: return this.render();
            case effectsSymbol$1: return this.runEffects(effectsSymbol$1);
        }
        this[phaseSymbol$1] = null;
    }
    render() {
...
function makeComponent$1(render) {
    class Scheduler extends BaseScheduler$1 {
        constructor(renderer, frag, host) {
            super(renderer, host || frag);
            this.frag = frag;
        }
...
const useEffect$1 = createEffect$1(setEffects$1);

const useContext$1 = hook$1(class extends Hook$1 {
    constructor(id, state, _) {
        super(id, state);
        this._updater = this._updater.bind(this);
        this._ranEffect = false;
...
function makeContext$1(component) {
    return (defaultValue) => {
        const Context = {
            Provider: class extends HTMLElement {
                constructor() {
                    super();
                    this.listeners = new Set();
                    this.addEventListener(contextEvent$1, this);
                }
                disconnectedCallback() {
                    this.removeEventListener(contextEvent$1, this);
                }
                handleEvent(event) {
                    const { detail } = event;
                    if (detail.Context === Context) {
...
const useMemo$1 = hook$1(class extends Hook$1 {
    constructor(id, state, fn, values) {
        super(id, state);
        this.value = fn();
        this.values = values;
...
const useLayoutEffect$1 = createEffect$1(setLayoutEffects$1);

const useState$1 = hook$1(class extends Hook$1 {
    constructor(id, state, initialValue) {
        super(id, state);
        this.updater = this.updater.bind(this);
        if (typeof initialValue === 'function') {
...
const useReducer$1 = hook$1(class extends Hook$1 {
    constructor(id, state, _, initialState) {
        super(id, state);
        this.dispatch = this.dispatch.bind(this);
        this.currentState = initialState;
...
function haunted$1({ render }) {
    const component = makeComponent$1(render);
    const createContext = makeContext$1(component);
    return { component, createContext };
}

const { component: component$1, createContext: createContext$1 } = haunted$1({ render: render$1 });
...
customElements.define('oma-site', component$1(Site));
